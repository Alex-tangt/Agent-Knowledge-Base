...
**适配层（Adapter）**：
对外暴露通道，与核心服务解耦。**当前唯一：`memory_agent`（记忆能力包）**。`legal_web`（政策法规问答 web）是**上一版本遗留的无关产品，明确排除**——仅保留代码，不优化 / 不依赖 / 不作对象（2026-09-16 owner 决定）。
_Avoid_: 把适配层逻辑混进核心服务、把 `legal_web` 当优化对象或回归锚点
...
（写不经过视图，如数据库的 VIEW 与基表）。软件能看见哪些视图 = 授权；网关的 tenant/密级
过滤是对视图的**强制收窄**（只可收窄，ADR-0018 D2）。最终可见 = 视图 ∩ 授权。
**v1 现状**：只有**未命名谓词接缝**（`payload_filter` / `writable_only`），**尚无具名视图与域寻址**（ADR-0025 D19）。
_Avoid_: 库、知识库（**技术语境一律改用「视图」**）、把视图当物理集合、假称具名视图已存在

**域（Domain）**：
一个带标签的**来源**（一个目录 / 仓库 / 工具自己的存储），**就地索引、不搬运**，有自己的
**所有者**（`owner`）。**v1 里域是只读的数据轴**：检索命中带 `owner`（**可见**），但**没有域寻址**
（读 = 整张基表，写只写唯一的全局知识库）——域寻址 / 具名视图后延（ADR-0025 D19）。只读域 = 无写者。
_Avoid_: 把域当物理集合、把域当视图、把工具参数 `section`（旧名 `domain`，= 分区，见下）当域

**全局知识库（Global Knowledge Base）**：
唯一**经工具可写**的域（`AGENT_KB_DIR`），**所有 agent 可读可写**；写经**写入网关** + 单 daemon
串行化（ADR-0025 D19）。非全局的域（各 agent 自己的工作区 / 仓库）由 **agent 自己写文件**，我们只读索引。
_Avoid_: 把全局知识库当基表全体、绕过写入网关直接改它的文件

**分区（Section）**：
`memory_add` / `memory_supersede` 的参数，指**全局知识库内**的目录分区（`topics` / `decisions` /
`projects/<slug>`）——**不是「域」**。历史命名为 `domain`，正名为 `section`（ADR-0025 D19）。
_Avoid_: 与「域」混用、用它指 owner 分区
...
**基表（Base Table）**：
"知识数据库"的基表 = **被收录的文件**（每条 = 一行）；真相源是各自的 Markdown 文件，索引是
**本地统一的派生向量索引**（一个本地集合 `memory_entries` 覆盖全部被收录文件）。整个系统本质是一个
数据库：表 / 视图 / 索引 / 所有权各有其位。注意**读全域、写一域**的不对称（ADR-0025 D19）。
_Avoid_: 把派生索引当真相源、把向量库当唯一源、以为读写两侧都有域寻址

**来源注册表（Source Registry）**：
**默认的收录来源清单**：每条来源带 `owner` 与默认 selection（路径模式 / 模板 glob），来源下的
文件自动继承该 `owner`。与"显式 overlay"合起来构成基表的 extent。
_Avoid_: 把注册表当唯一收录途径（用户可显式追加）

**收录（Ingestion / Admission）**：
决定**基表 extent** 的动作（**DDL**，不是写数据）：注册表默认 ∪ 显式 overlay（**独立清单**）；
粒度 = **路径模式**（精确文件 / 窄 glob；收整棵子树须配 exclude）。`owner` 默认按来源，特殊收录
可显式指定。与 `memory_add`（DML，写一行）是两件事。
_Avoid_: 把收录当写记忆、把整棵子树无 exclude 地收进来

**写入网关（Write Gateway）**：
强制"先检索去重 → 写 → 校验"的写入通道；新增可自动，替代/归档类破坏性操作需确认。
**v1 只服务全局知识库**（唯一经工具可写的域），且**对所有 agent 开放**；其它域由 agent 自己写文件，
不经过它（ADR-0025 D19）。**写侧没有域寻址**。
_Avoid_: 裸文件写、把写入网关当基表的通用写通道 / 加写侧域路由
