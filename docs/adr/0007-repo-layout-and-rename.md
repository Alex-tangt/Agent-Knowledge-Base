...
- **未完成（已在第二轮补齐）**：仓库与本地目录更名 `Agent-Knowledge-Base`。
...
- **本地目录改名：由用户在会话外完成**。会话内 `Rename-Item` 连续两次失败（共享冲突）：本机多个 `opencode.exe` 各自派生 `markitdown-mcp.exe`（stdio MCP，CWD = 仓库目录），锁住目录；不在会话内杀进程（可能波及其他会话）。用户改目录 + 重启会话后验收通过。
...
- **新路径验收（2026-09-14）**：`venv\Scripts\python.exe -m pytest tests/unit -q` → **71 passed**；legal_web 导入冒烟 → **import-ok**；启动冒烟见 `memory_agent/eval/baseline_A.md`。`legal_web/data/SOURCES.md`、`legal_web/kb_registry.json` 均无硬编码绝对路径，无需迁移。
