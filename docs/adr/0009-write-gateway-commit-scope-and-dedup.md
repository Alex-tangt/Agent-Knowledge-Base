...
D3 的阈值已在真实 KB 上校准：**`DEDUP_THRESHOLD` 0.92 → 0.88**。
实验与逐条分数见 `experiments/dedup-threshold-calibration/`（`README.md` + `scores.json`），
sandbox 套件见 `memory_agent/eval/write_path_sandbox.py`。

- 方法：真实 KB 提交态（git clone）建可写条目索引，对每条目用与写入网关**逐字一致**的查询
  （`f"{title}\n\n{body}"`）及去标题前缀变体检索，记录自匹配分（正例）与最近邻分（负例）。
- 21 条实测：正例（去标题重加）min 0.993、精确重加 min 0.949；负例（不同条目最近邻）max 0.792。
- 依据：精确重加 0.949 距 0.92 仅 0.029，措辞略变即漏报；负例最高 0.792，0.88 仍有 0.088
  误报余量。0.88 取在实测间隔中部，扩大同义重加捕获带而不触实测负例。
- 边界：无 LLM 同义改写样本（改写需模型、非确定性），正例为「去标题前缀」代理；故 0.88 仍
  偏保守。后续若引入改写样本应复校。
