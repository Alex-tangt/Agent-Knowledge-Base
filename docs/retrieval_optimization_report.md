...
| ⑦ ⑥+手写 kw（**蒸馏拟合**，colbert512/z-score `[.45,.50,.05]`） | `fit_fusion.py`+`ablate_signals.py`，实测 2026-09-16 | **0.7981** | **0.9130** | **0.9685** | **0.9282** | **+4.1 题，bootstrap 显著** |
...
| U3 | **ColBERT + 蒸馏融合落地（⑤⑦⑧）** | **已测并过闸门（3 路即可）**：`colbert512+dense+kw / z-score / [.45,.50,.05]` → **r@1 0.7981（+4.1 题）/ r@5 0.9685 / nDCG 0.9282**，bootstrap CI [+0.078,+0.294] 显著；`sparse` 已消融剔除；**不需要 query 自适应**（oracle headroom −0.087） | 结构性（multivector schema `MAX_SIM` + **260MB@512**（1 万条约 19GB，int8 可÷4）+ 全量重建）；`dense`/`kw` 零增量。建议形状 = **dense+kw 召回 → colbert 池内重打分**。**待 owner 决定是否开票** |
