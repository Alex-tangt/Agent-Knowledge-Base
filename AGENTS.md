...
- **融合参数蒸馏拟合（2026-09-16，`experiments/bge-m3-sparse-colbert/`）**：用 **51×134 全量 rerank 分数**（jina int8，一次录 13 min）作监督，目标 = 让融合排序逼近 rerank 排序（凸代理 + 网格交叉验证），**qrels 只作验证**。结果：`colbertfull+dense+kw+sparse / z-score / [.40,.45,.10,.05]` → **r@1 0.7759（+3.1 题）/ r@3 0.891 / r@5 0.9685 / nDCG@10 0.9143 / MRR 0.9144**，paired bootstrap Δ(r@1) CI [+0.059,+0.235] **显著**；**不需要 query 自适应**（oracle headroom −0.087）。BGE-M3 **sparse 否决**（0.133）、**colbert 正向**（单路 0.698；**512 截断 = full**，存储 281MB 而非 1.02GB）。落地需 Qdrant **multivector** schema → **默认链路变更待 owner**。正文来源必须走索引 payload（见 Gotchas）。
