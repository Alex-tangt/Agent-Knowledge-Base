...
memory_agent/       # 记忆能力包（MCP + skill）；可安装
  ...
  gateway/  corpus/  memory/  parse/  skill/  agent/  plugin/  eval/  requirements.txt  vector_db/  README.md
  # agent/ = 随包交付的 opencode subagent 定义（memory-research；#60 / ADR-0026 D10）
  # plugin/ = 随包交付的 opencode 插件（memory-research → 工具 memory_research，形态 B；#61）
  ...
  # opencode_config.py = opencode 注册 + skill/agent/plugin 落位（零第三方依赖，bootstrap 阶段可用）；cli.py = `memory-agent install` / 无子命令=MCP server
...
### memory_agent (记忆能力包)
- **可安装（#26 / ADR-0024）**：`pip install -e ragcore -e memory_agent`（Venv 段已含）。等价的模块/脚本入口：`python -m memory_agent.mcp_server`、`python -m memory_agent.build_index`，以及 console scripts `memory-agent` / `memory-agent-proxy` / `memory-agent-build-index` / `memory-agent-connect`。脚本入口（`memory_agent/*.py`）安装后从任意 CWD 均可运行。`memory-agent` 现为统一 CLI：`memory-agent install` = 一键部署（`cli.py` → `deploy.py`），`memory-agent uninstall` = 卸载 opencode 注册/skill + 停 daemon（不删 clone；幂等+备份+`--dry-run`），无子命令仍是 MCP server（向后兼容）。
- **一键部署（#52 / ADR-0028）**：`git clone` 后 `bash install.sh`（Linux/WSL/macOS）或 `powershell -File install.ps1`（Windows；系统自带 PowerShell 5.1 即可，装了 `pwsh` 也行）——薄壳用系统 Python 起 `python -m memory_agent.deploy install`（顶层零第三方依赖，可在 venv 建好前跑）。编排：建 venv → 装 `memory_agent/deploy-requirements.txt`（**与 `legal_web` 解耦**，D3）→ editable 装两包 → 建索引 → **写 opencode 注册**（`~/.config/opencode/opencode.json`，幂等 + 备份 + `--dry-run`，D4）→ 落 skill → 起 daemon → 冒烟。开关：`--dry-run` / `--no-index` / `--no-daemon` / `--no-smoke` / `--with-tests` / `--force-index`；Linux/Windows 默认装 CPU 版 torch（`--no-cpu-torch` 可回 PyPI 默认）。POSIX 守护：`proxy._spawn_daemon` 在非 Windows 用 `start_new_session=True`（D1）。
...
- **第二消费者共享同一 daemon（#41 / #43 / ADR-0025 D17-D18）**：`python -m memory_agent.connect`（console `memory-agent-connect`）一步把 DeepTutor 接到**同一个共享 daemon**——写它的部署级 `mcp.json`（`streamableHttp → http://127.0.0.1:8765/mcp`，共享工具白名单）与 `model_catalog.json`（embedding profile `binding=vllm` → `http://127.0.0.1:8765/v1/embeddings`、`dimension=1024`、免 key；**保留** LLM profile 与其 key），落位 skill，核验 opencode 注册（缺则打印 patch、不改用户配置），幂等确保 daemon 在跑。**不新建服务端、不各建索引**：MCP 共享基表 + 派生索引，embedding 共享 daemon 里那份 BGE-M3（daemon 的 `POST /v1/embeddings`，OpenAI 兼容；authn 与 `/mcp` 同源，未配 token 即零配置直连）。第二消费者 **可读可写**（`ADR-0025` **D19** 修订 D17，**#44 已返工**）：`enabled_tools` = 读 + 内容写（`add`/`supersede`/`archive`），**不含** `reindex` 与收录 DDL（`ingest_include`/`ingest_exclude`）；写入**问责面 = agent 身份**（审计每条 `tools/call`）。证据 `memory_agent/eval/shared_service_41.py`（**37/37**）、`bge_m3_embeddings_43.py`（**17/17**）。
...
- The memory index uses its **own** Qdrant path (`memory_agent/vector_db/<gen>/qdrant`, resolved via the `CURRENT` pointer), and the client is opened **per operation** (no long-held lock) — it can coexist with `legal_web`; see ADR-0008 D5. Full rebuilds build a new generation and atomically swap the pointer; an interrupted rebuild never leaves "empty index + stale manifest" (ADR-0011).
  - **当前形状（2026-09-16 晚，ADR-0019 D16）**：`CURRENT = gen-4`，集合 = **具名 `dense` + `sparse`**（hybrid），**188 条**；词法线 = **BM25**（fastembed 客户端编码 + store 端 IDF），融合 = **DBSF**；手写关键词加法已退出记忆默认路径（`MEMORY_LOCAL_HYBRID=0` 可回）。改动索引 schema 后**必须重建/迁移**，否则 store 与集合形状不匹配。
  - **提速技巧**：换词法编码器不必重嵌 dense——`experiments/local-lexical-40/build_hybrid.py` 从旧集合 `scroll(vectors)` 复用 dense、只补算 sparse，再 `refresh()` 增量追平新增/变更条目（本次 188 条总耗时 ~8 min，其中 124 条跳过）。
...
- **确定性检索评测基座（#24 / 叙事 #21）**：对象 = **记忆检索**（不是法律 RAG）；目标链路 = 向量 + 词法（2026-09-16 起默认 **BM25**，经 store 原生 hybrid）+ 可选 rerank，走 `MemoryIndex.search` → `MemoryRetriever` → store 原生融合（或 `ragcore` 策略）+ `RerankerService`，不调 LLM。评测集 `memory_agent/eval/retrieval_eval_set.json`（51 条 = 45 有答案 + 6 无答案，条目级二值）；harness `memory_agent/eval/retrieval_eval.py`（`--mode vector|hybrid|hybrid-rerank`，`--trace` 断点续跑，`meta.run_hash` 验确定性）。基线见 `memory_agent/eval/retrieval_baseline.md`。无答案 query 只作描述性观察、不校阈值（ADR-0017）。
- **融合参数蒸馏拟合（2026-09-16，`experiments/bge-m3-sparse-colbert/`）**：用 **51×134 全量 rerank 分数**（jina int8，一次录 13 min）作监督，目标 = 让融合排序逼近 rerank 排序（凸代理 + 网格交叉验证），**qrels 只作验证**。结果（**3 路**）：`colbert512+dense+kw / z-score / [.45,.50,.05]` → **r@1 0.7981（+4.1 题）/ r@3 0.9130 / r@5 0.9685 / nDCG@10 0.9282 / MRR 0.9274**，paired bootstrap Δ(r@1) CI [+0.078,+0.294] **显著**；**不需要 query 自适应**（oracle headroom −0.087）。BGE-M3 **sparse 否决**（单路 0.133；消融 Δ(4−3) r@1 CI **[−0.059, 0.000]** → 加了只会更贵）、**colbert 正向**（单路 0.698；**512 截断≈full**，**成本全在此路**：260MB/134 条 @512，1 万条约 19GB，int8 可÷4；`dense`/`kw` 零增量）。落地需 Qdrant **multivector** schema + **dense+kw 召回 → colbert 池内重打分** → **默认链路变更待 owner**。正文来源必须走索引 payload（见 Gotchas）。
  **后续（同日）**：owner **放弃 colbert（存储不可接受：260MB/134 条 @512，1 万条约 19GB）** → 该显著增益**不落地**；三条词法路（手写 kw / **BM25** / BGE-M3 sparse，生态位相同**只放一条**）对照：**单路 BM25 最强**（r@1 **0.411** vs kw 0.167 vs sparse 0.133，IDF 是关键），但 **`dense+L` 融合里三者两两不显著**（Δr@1 的 95% CI 全含 0）、也都不显著优于生产 0.7074；**归一化必需**（BM25 原始分 max **34.06** vs dense ~0.78 → 不归一化只能把词法权重压到下限 0.05；zscore 后可达 0.15）。→ **默认链路维持**（换 BM25 的价值是标准化/免自研，不是可测增益）。
...
- **实验 / 验证若有 rerank 需求 → 用新模型**（owner 2026-09-16）：`MEMORY_RERANK_BACKEND=onnx`（jina-reranker-v2 int8，实测 **2.67s/题 vs m3 11.68s = 4.4x**，recall@5 与 m3 相同）；不要再拿 m3（torch）当实验默认重排器。m3 只在需要"许可干净 / legal 锚点不动"时用。
...
**再决定**：直接用 / 适配 / 自己优化。理由：自造基线成本高、易带出题或实现偏差；开源基线给出
**外部可比锚点**与**已达成的下界**。

- 顺序：**调研 → 复用基线 → 廉价测量 → 才排贵运行**（与「廉价测量优先」互补）。
- 复用 ≠ 照搬：**记录**基线来源、版本 / commit、它的指标与边界，再写我们的适配点。
- 先例：agentic RAG 的「迭代检索」选 **MultiHop-RAG** 及其自带检索 / 评测脚本作基线，
  而非自造多跳集（ADR-0026 D2；#47 / #48）。
- 反模式：跳过基线直接自造 → 无法与外部比较，也说不清"到底做到什么水平"。
...
- **一次记录，离线分析（2026-09-16）**：把每个信号的**完整打分矩阵**（query × 全部条目）落盘为**不可变证据**，之后所有选型（归一化 / 权重 / 子集 / k / 阈值 / 拟合）都在矩阵上做**纯算术**——**不再重跑模型或检索**。先例：`experiments/bge-m3-sparse-colbert/`（`cache/*.npz` 百 KB + `fit_fusion.py`；**rerank 分数矩阵 51×134 一次录 13 min，之后永久复用**）。**何时才必须重跑**：① 表示本身变了（换编码器 / chunking / 截断）；② 要验端到端；③ 语料或题集变了。**"记录"与"跑测试"是两件事。**
- **别用 45 个二值标签调参**：小样本网格会出现"上百个组合都能打赢基线"（2026-09-16 实测 88/1144）——那是**过拟合信号**，不是"很多好方案"。有客观连续信号（如 rerank 分数）时，**用连续监督拟合 + qrels 只作验证**（`fit_fusion.py`），并用 paired bootstrap 判显著。**目标函数必须 top 加权**（全语料权重会被尾部无关条目主导：全局 Spearman 仅 ~0.46）。
...
- **下一阶段 = agentic RAG（agent loop 做深）**：**先量检索头寸、再谈机制 / 工具**（`ADR-0026` accepted）；✅ **#47 Phase A**（MultiHop-RAG 单次证据 census，**D4 = 有头寸**：recall@5 0.650 / @50 0.965，**排序头寸为主**；证据 `experiments/agentic-rag-census/`）→ ✅ **#48 Phase B**（三臂，N=200、`qwen3.7-flash`）：**迭代有效**（B3 答案 +10.2pp / recall@5 +9.1pp，均显著；增益主落「排序 miss」；两跳拿走大部分）；**裸改写无增益**（B2−B1 CI 含 0）；**LLM 早停 35.8%**；`ADR-0026` D5：机制证据、非产品增益。证据 `experiments/agentic-rag-census/phase_b/`。
  ...
  → ✅ **#60 委派子代理（feat/60-memory-research-subagent）**：宿主能力核实（opencode **不支持 MCP sampling**——`mcp/index.ts` 注释掉；有原生 subagent + `steps`）→ 拍板 `ADR-0026` **追加 D9–D12**：把多跳检索放进 `mode: subagent` 的**独立子会话**（不污染主对话）。随包交付 `memory_agent/agent/memory-research.md`（默认**不写 model = 继承**；`"*": deny` 在前 + 只读记忆工具 allow 在后）；`memory-agent install/uninstall` 落位/移除（`--no-agent`）。**上下文隔离 + 成本上限，不修早停、不验 skill**。实验变体（钉 qwen3.7-flash）与前置见 `experiments/agent-loop-subagent/`。
  → ✅ **#61 形态 B（插件工具，feat/61-...）**：spike 证实（`experiments/opencode-plugin-subagent/`，PASS，日志可核）"插件 `execute` 内 `session.create({parentID}) + session.prompt({agent})` 不重入/不死锁" → `ADR-0026` **追加（2026-09-21）采纳形态 B**：随包 `memory_agent/plugin/memory-research.js` 暴露工具 **`memory_research(query, max_hops=2)`**——**阻塞**（同 Task 前台）、**只回文本**、**hop 预算由插件控**（`NEXT_QUERY:` 协议）、`metadata` 进度 + `abort`；落全局 `~/.config/opencode/plugins/` + 依赖写 `~/.config/opencode/package.json`（`--no-plugin`）；形态 A 保留 fallback。
...
- ✅ **一步安装 + 第二消费者（#41，feat/41-shared-service）**：新增 `memory_agent/connect.py`（`python -m memory_agent.connect`，console `memory-agent-connect`）——写 DeepTutor **部署级** `<DEEPTUTOR_HOME>/data/user/settings/mcp.json` 一条 `streamableHttp → http://127.0.0.1:8765/mcp`（`--dry-run`；保留其它条目 / 原子写 / 幂等）、落位 skill、核验 opencode 注册（缺则打印 patch、不改用户配置）、幂等确保 daemon 在跑；**不新建服务端 / 不各建索引**（同一 daemon = 同一基表 + 派生索引）。第二消费者边界后由 **#44 依 D19 改为共享可读写**（`enabled_tools` = 读 + 内容写；**原 D17 只读口径作废**）。验收：`shared_service_41.py` **37/37**（`memory_agent/eval/shared_service_41_results.md`；两传输同 `gen`/同 top-k、D9 惰性追平、跨消费者双向写→读、审计归属 agent；Stub 嵌入确定性）。
- ✅ **第二消费者 embedding 绑到 BGE-M3（#43，feat/43-bge-m3-embeddings）**：共享 daemon 新增 **OpenAI 兼容 `POST /v1/embeddings`**，复用**进程内唯一一份 BGE-M3**（新增 `ragcore/services/embedding_provider.py` 单例，顺带修「每次换代重建 store 就重载权重」）；`connect.py` 幂等写 DeepTutor `model_catalog.json` 的 embedding profile（`binding=vllm` → `/v1/embeddings`、`dimension=1024`、免 key，**保留 LLM profile 与其 key**）；authn 与 `/mcp` 同源（未配 token 零配置直连）。钉 `transformers==4.57.6`（ST 5.4.1 在 5.5.x 下对 `BAAI/bge-m3` 的 `AutoProcessor` 抛错）。验收：`pytest tests/unit -q` **374 passed / 2 skipped / 3 xfailed** + `bge_m3_embeddings_43.py` **17/17**（`memory_agent/eval/bge_m3_embeddings_43_results.md`；含 **DeepTutor 自身 `EmbeddingClient`** 端到端取到 2×1024 向量）。
...
- **索引 payload 里的正文 ≠ 原始 `.md` 文件**：索引存的是**渲染正文**（frontmatter 剥离、标题变 `# ...`），而 `manifest[entry]["path"]` 指向的原始文件**带 YAML frontmatter**。任何"重新编码 / 重排"的实验若读原始文件，送排的前 512 字全是 frontmatter → 信号与生产**完全不一致**（实测：doc token 1860→1339、sparse 非零 395→89、rerank-only r@1 0.7648→0.8537）。**取正文一律走索引 payload**（参考 `experiments/bge-m3-sparse-colbert/docs_source.py`，内含 frontmatter 断言）。
