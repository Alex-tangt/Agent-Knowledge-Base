...
- **下一阶段 = agentic RAG（agent loop 做深）**：**先量检索头寸、再谈机制 / 工具**（`ADR-0026` accepted）；执行票 **#47 Phase A**（MultiHop-RAG 单次证据 recall census）+ **#48 Phase B**（迭代检索三臂，blocked by #47）。范围冻结：只 skill + 读侧确定性工具，**不碰检索默认 / 合成**。
