...
- **第二消费者共享同一 daemon（#41 / #43 / ADR-0025 D17-D18）**：`python -m memory_agent.connect`（console `memory-agent-connect`）一步把 DeepTutor 接到**同一个共享 daemon**——写它的部署级 `mcp.json`（`streamableHttp → http://127.0.0.1:8765/mcp`，只读工具白名单）与 `model_catalog.json`（embedding profile `binding=vllm` → `http://127.0.0.1:8765/v1/embeddings`、`dimension=1024`、免 key；**保留** LLM profile 与其 key），落位 skill，核验 opencode 注册（缺则打印 patch、不改用户配置），幂等确保 daemon 在跑。**不新建服务端、不各建索引**：MCP 共享基表 + 派生索引，embedding 共享 daemon 里那份 BGE-M3（daemon 的 `POST /v1/embeddings`，OpenAI 兼容；authn 与 `/mcp` 同源，未配 token 即零配置直连）。第二消费者 MCP v1 **只读**（`enabled_tools` 白名单），写仍只走本产品写入网关。证据 `memory_agent/eval/shared_service_41.py`（**27/27**）、`bge_m3_embeddings_43.py`（**17/17**）。
...
- **主线 = 个人模式**（唯一开发重点）：✅ **#41 一步安装 + 第二消费者**（深·共享服务，`ADR-0025` D17）· ✅ **#43 embedding 绑本机 BGE-M3**（`ADR-0025` D18）· **#42 agent loop** · 收尾。
