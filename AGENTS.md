...
- **记忆检索评测基座（#24，需 BGE-M3 [+ reranker]）**：`venv\Scripts\python.exe memory_agent/eval/retrieval_eval.py --mode hybrid-rerank`——不在 `tests/unit` 内（运行时证据），基线见 `memory_agent/eval/retrieval_baseline.md`。
