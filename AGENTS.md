...
### memory_agent (记忆能力包)
- **可安装（#26 / ADR-0024）**：`pip install -e ragcore -e memory_agent`（Venv 段已含）。等价的模块/脚本入口：`python -m memory_agent.mcp_server`、`python -m memory_agent.build_index`，以及 console scripts `memory-agent` / `memory-agent-proxy` /
`memory-agent-build-index` / `memory-agent-connect`。脚本入口（`memory_agent/*.py`）安装后从任意 CWD 均可运行。
...
- **第二消费者共享同一 daemon（#41 / ADR-0025 D17）**：`python -m memory_agent.connect`（console `memory-agent-connect`）一步把 DeepTutor 接到**同一个共享 daemon**——它在 DeepTutor **部署级** `<DEEPTUTOR_HOME>/data/user/settings/mcp.json`（缺省 home = `$DEEPTUTOR_HOME`，否则 CWD，与其自身 `get_runtime_home` 一致）写一条 `streamableHttp → http://127.0.0.1:8765/mcp`（保留其它条目 / 原子写 / 幂等 / `--dry-run`），落位 skill，核验 opencode 注册（缺则打印 patch、**不改用户配置**），并幂等确保 daemon 在跑。**不新建服务端、不各建索引**；**v1 只读**——用 DeepTutor 侧 `enabled_tools` 白名单（`memory_search`/`memory_get`/`memory_index_status`/`memory_ingest_list`）落实，写仍只走本产品写入网关。证据 `memory_agent/eval/shared_service_41.py` + `..._results.md`（**27/27**，Stub 嵌入的确定性验收）。
...
## Tests
- **单元测试**：`tests/unit/`（pytest）——`test_document_service.py`、`test_rag_service.py`、`test_session_memory.py`、`test_memory_corpus.py`、`test_memory_index.py`、`test_memory_index_qdrant.py`、`test_memory_reindex.py`、`test_memory_writer.py`、`test_memory_lazy_warmup.py`、`test_memory_concurrency.py`、`test_memory_proxy.py`、`test_mcp_server_cli.py`、`test_config_layering.py`、`test_vector_store_clear.py`、`test_vector_store_filter.py`、`test_vector_store_locking.py`、`test_retrieval_strategy.py`、`test_memory_retrieval.py`、`test_memory_store_port.py`、`test_eval_metrics.py`、`test_reranker_service.py`、`test_gateway_authz.py`、`test_hf_offline.py`、`test_memory_selection.py`、`test_memory_admission.py`（后两个是 #36 收录 / 运行时选择）、`test_connect.py`（#41 一步安装纯函数）。运行：`venv\Scripts\python.exe -m pytest tests/unit -q`（350 passed / 2 skipped / 3 xfailed）。
...
- ✅ **一步安装 + 第二消费者（#41，feat/41-shared-service）**：新增 `memory_agent/connect.py`（`python -m memory_agent.connect`，console `memory-agent-connect`）——写 DeepTutor **部署级** `<DEEPTUTOR_HOME>/data/user/settings/mcp.json` 一条 `streamableHttp → http://127.0.0.1:8765/mcp`（`--dry-run`；保留其它条目 / 原子写 / 幂等）、落位 skill、核验 opencode 注册（缺则打印 patch、不改用户配置）、幂等确保 daemon 在跑；**不新建服务端 / 不各建索引**（同一 daemon = 同一基表 + 派生索引）。**只读边界**（D17）由 DeepTutor 侧 `enabled_tools` 白名单（`memory_search`/`memory_get`/`memory_index_status`/`memory_ingest_list`）落实。本机已实际接入（`D:\Study\my-deeptutor`，用 DeepTutor 自身 `load_mcp_config` 复核）。验收：`pytest tests/unit -q` **350 passed / 2 skipped / 3 xfailed** + `shared_service_41.py` **27/27**（`memory_agent/eval/shared_service_41_results.md`；两传输同 `gen`/同 top-k、D9 惰性追平、跨消费者写→读、DeepTutor 自身函数校验；Stub 嵌入确定性）。
