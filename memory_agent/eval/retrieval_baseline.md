...
> **注意（#30，2026-09-15）**：上表 `hybrid` 行是**旧（关键词优先）融合**的存档。默认融合已改为
> **加法关键词增强**（`docs/adr/0022` D4），`--mode hybrid` 现为 recall@1 **0.7074** / nDCG@10
> **0.8817** / MRR **0.8731**（pool 14）。新对照与池曲线见 `experiments/fusion-selection/`。
> `hybrid-rerank` 基线（0.8593/0.9658）在融合切换后**基本不变**（rerank 与融合顺序解耦）。
