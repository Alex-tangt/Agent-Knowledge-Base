...
```
memory_agent/
├── mcp_server.py          # stdio MCP：memory_search / memory_get / memory_add  (#10,#11)
├── runtime.py             # 先立 stderr 日志，再装配索引 / 写入网关单例
├── _bootstrap.py          # stdio 安全日志 + ragcore sys.path 垫片
├── settings.py            # 真相源 / 索引路径 / 集合名 / 去重阈值（勿命名 config.py，会遮蔽 ragcore 的 config 包）
...
├── memory/writer.py       # 写入网关：搜索→去重→校验→落盘→路径级 git commit  (#11)
...
生命周期工具（`memory_supersede` / `memory_archive`）与索引增量一致性（hash 跳过 / 孤儿清理）
分别是 #12 与 #13。写入后**不刷新索引**（属 #13），故新条目在重建前对检索不可见。
...
- `memory_add(title, body, domain, type, tags, slug, sources, status, allow_duplicate)` → 唯一写入口；写前去重，命中近似条目则**不写**并返回候选（`allow_duplicate=true` 是误报出口）；写入 = frontmatter 过校验 + 一个**只含本条目文件**的 git commit。
