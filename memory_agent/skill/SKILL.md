...
## 委派：多跳检索交给子代理（#60 / #61 / ADR-0026 追加）

主对话里自己跑迭代会把 hop / 召回块 / 判断过程灌进**主上下文**。**优先**把多跳 / 多约束
检索**委派**出去，两条路：

- **有 `memory_research` 工具时（推荐，形态 B）**：直接调它——插件（#61）在**独立子会话**里
  驱动 `memory-research` 子代理，**hop 预算由代码控**、**阻塞**返回（同 Task 前台）、检索过程
  不回流主对话。`query` 需自足。
- **没有该工具时（形态 A，fallback）**：用宿主 subagent（opencode：Task 工具 → 独立子会话）
  委派同一子代理。

下面的规则对两条路都适用：

1. **主 agent 先把 query 补自足**——子代理**看不到主对话**，代词 / 省略 / 上文依赖必须
   由主 agent 消解后再传入。
2. 子代理在自己子会话里按上面的「迭代检索」规则跑，**只把结论 + `id`** 交回主对话。
3. 子代理**只读**（无写工具）；写仍由主对话走 `add` / `supersede` / `archive`，且破坏性
   操作要先经用户确认。
4. 宿主**没有** subagent（或不需要隔离）时，退回本节之上「迭代检索」自跑。

> 子代理定义随包交付，`memory-agent install` 落位到 `~/.config/opencode/agents/`。
> 它是**上下文隔离 + 成本上限**（agent `steps`，部署建议），**不改检索链路、不修早停**、
> 也不构成对上述 skill 规则的验证。
...
- 设计：`docs/adr/0008`（读路径）、`0009`（写入网关）、`0010`（生命周期工具）、
  ...
  `0026`（迭代检索：测量协议 + 本节的规则来源；追加节 D9–D12 = 委派子代理
  `memory-research`，见 #60）。
