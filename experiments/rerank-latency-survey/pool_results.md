...
1. **池子是降延迟主杠杆，且可无损裁剪**：hybrid 池 **10**（无损上限）/ **12**（安全默认，覆盖 max first-hit=12）。
...
- **未实测「裁池后的 rerank 质量」**：本页是**上限论证**（ceil 不变），但 reranker 在更小池上的实际 nDCG@10 需**跑一次** `--mode hybrid-rerank`（池 10 或 12）确认 **nDCG@10 ≥ 0.9658**。
- 语料增长后需重跑（索引变化 → 曲线变化）。
