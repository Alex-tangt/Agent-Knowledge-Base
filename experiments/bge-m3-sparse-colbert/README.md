...
**`@N` 的含义与线性外推**（`@N` = 每条条目只保留**前 N 个 token** 的向量；条目数恒为 134）：
`存储 = Σ_每条 min(token 数, N) × 1024 × 字节/维` → **条目数翻倍即存储翻倍**（长度分布稳定时）。

| 方案 | 单条成本 | 1,000 条 | 10,000 条 |
|---|---|---|---|
| 全量 fp32（均 1339 token / max 3798） | 5.48 MB（最长一条 15.6MB） | ~5.5 GB | **~55 GB** |
| **@512 fp32**（本语料 109/134 条被截） | **2.10 MB**（满 512；均 1.94） | ~1.9 GB | **~19 GB** |
| @512 **int8** | 0.52 MB | ~0.5 GB | ~5.2 GB |
| @128 fp32 | 0.52 MB | ~0.5 GB | ~5.2 GB |
| *（对照）dense* | *4 KB* | *4 MB* | *40 MB* |

- **只有 `@N` 截断能把斜率钉死**（不截断时单条成本随条目长度走）；colbert ≈ dense 的 **500 倍**（@512 fp32）。
- **查询期**成本与 D 无关（池内重打分），但**存储**无法回避（MaxSim 需要全部候选的 token 向量）。
- 降斜率三旋钮：**截断 N** / **量化**（int8 ÷4，二值再降）/ **只对高价值域索引 colbert**。
...
## 11. 三条词法路选一条：手写 kw vs **BM25** vs BGE-M3 原生 sparse（2026-09-16）

> owner 决定**放弃 colbert**（存储不可接受，见 §10.2）；三条词法路**生态位相同** → 融合里**只放一条**。
> 融合形态 = `dense + L`（L ∈ {kw, bm25, sparse}）；**归一化作为候选维度**（量纲差异）。
> 目标函数仍为蒸馏 nDCG@5（对全量 rerank 分数）；qrels 只作 L3 验证 + paired bootstrap。
> 产物：`bm25_matrix.py`（Qdrant `modifier=Idf` 口径，生产同路径）· `cache/bm25.npz` · `compare_lexical.{json,md}`。

### 11.1 锚点

`dense + kw / none / [0.95, 0.05]` 逐位复现生产 **0.7074 / 0.8685 / 0.9185 / 0.8817 / 0.8731** → 网格与生产口径一致。

### 11.2 单路（词法各自独跑）：**BM25 完胜**

| 单路 | distill5 | r@1 | r@3 | r@5 | nDCG@10 | MRR |
|---|---|---|---|---|---|---|
| **bm25** | **0.5226** | **0.4111** | 0.5444 | 0.6111 | 0.5847 | 0.5370 |
| kw（手写） | 0.4864 | 0.1667 | 0.4111 | 0.4778 | 0.3865 | 0.3462 |
| sparse（BGE-M3） | 0.3332 | 0.1333 | 0.2222 | 0.2667 | 0.2520 | 0.2382 |

→ **BM25 是唯一像样的独立词法信号**（r@1 0.411 = kw 的 2.5 倍、sparse 的 3 倍）。IDF 是关键：kw/sparse 都缺它。

### 11.3 `dense + L` 融合：**三者统计上不可区分**

各词法的最优（按 distill5；权重由 0.05 步长网格选出）：

| 融合 | distill5 | r@1 | r@5 | nDCG@10 | MRR | w |
|---|---|---|---|---|---|---|
| **dense + bm25 / zscore** | **0.8088** | 0.6963 | 0.9407 | 0.8801 | 0.8484 | [0.85, 0.15] |
| dense + bm25 / minmax | 0.8066 | 0.6963 | 0.9407 | 0.8801 | 0.8484 | [0.90, 0.10] |
| dense + kw / minmax | 0.8032 | 0.6741 | 0.9185 | 0.8671 | 0.8398 | [0.95, 0.05] |
| dense + kw / none（**=生产**） | 0.8029 | **0.7074** | 0.9185 | **0.8817** | **0.8731** | [0.95, 0.05] |
| dense + sparse / minmax | 0.7986 | 0.6630 | 0.9185 | 0.8523 | 0.8131 | [0.90, 0.10] |
| dense + bm25 / none | 0.7932 | **0.7111** | 0.9407 | 0.8662 | 0.8330 | [0.95, 0.05] |
| *（参考）dense+kw+bm25* | *0.8104* | *0.7296* | *0.9407* | *0.8937* | *0.8854* | *[0.8,.05,.15]* |

**paired bootstrap（对 51 题，B=2000）**：

| 比较 | Δr@1 的 95% CI | 判定 |
|---|---|---|
| bm25 最优 − 生产 | [−0.1373, +0.0588] | **不显著** |
| kw − bm25 | [−0.0784, +0.0392] | 不显著 |
| bm25 − sparse | [−0.0588, +0.1373] | 不显著 |
| kw − sparse | [−0.0392, +0.0980] | 不显著 |

→ **在 dense 已经提供主要信号的前提下，换哪条词法路都不显著**；三者两两之间也都不显著。
"dense+kw+bm25"（同时放两条，违反生态位假设）在 qrels 上看着最高（r@1 0.7296），但同样落在噪声带内。

### 11.4 归一化的作用（证实 owner 的判断）

**不归一化时，BM25 的原始分最大 34.06**（dense 仅 ~0.78）→ 网格只能把它压到权重下限 0.05 才不淹没 dense：

| 归一化 | bm25 最优权重 | distill5 |
|---|---|---|
| none | **0.05（被量纲逼到下限）** | 0.7932 |
| minmax | 0.10 | 0.8066 |
| **zscore** | **0.15** | **0.8088** |

→ **归一化让词法权重从"被迫贴地"升到能真正起作用（0.15）**，蒸馏目标随之提升。
`kw` 反而在未归一化时最好（原始口径 = 生产），因为它自带"命中词数占比"的有界性（[0,1]）。

### 11.5 结论

1. **单路质量：BM25 ≫ kw > sparse**（r@1 0.411 / 0.167 / 0.133）→ 要留一条词法路，**BM25 是唯一像样的选择**。
2. **融合里换词法路不显著**：所有 pairwise CI 都含 0，且都不显著优于现生产（0.7074）。
   最优点估计 `dense+bm25/zscore/[.85,.15]` 蒸馏口径 0.8088 > 生产 0.8029，但 **r@1 反而略低**（0.6963 vs 0.7074）
   → **目标函数与 qrels 在这个粒度上不一致，说明差异就是噪声**。
3. **所以**：换 BM25 的收益是**"标准化 + 免自研 + 独立质量强 2.5×"**（可维护性/鲁棒性），
   **不是可测的融合增益**；且代价 = 需 hybrid 集合（schema 变更）+ `fastembed`（已有可选 extra）。
4. **放弃 colbert 的代价要记账**：本轨唯一"显著净胜"的方案（3 路含 colbert512，r@1 0.7981/+4.1 题）
   因存储被弃 → **剩余可调项在 45 题上测不出显著提升**，默认链路（0.7074）维持即可。

Relates：§9/§10、`docs/retrieval_optimization_report.md` §C2、ADR-0019 D14/D15（BM25 可选后端）。
